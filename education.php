...
<form action="add_education.php" method="post">
    <input type="text" name="degree" placeholder="level" required>
...
<ul class= "ll">
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<li>" . htmlspecialchars($row['degree']) . " from " . htmlspecialchars($row['institution']) . " (" . $row['start_date'] . " to " . $row['end_date'] . ") <a href='delete_education.php?id=" . $row['id'] . "'>Delete</a></li>";
    }
    ?>
</ul>
</div>
